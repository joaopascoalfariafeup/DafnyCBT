...
class Program
{
        ...
        List<Expression> preLits, Method method, HashSet<string> mutableNames, string z3Path)
    {
        ...
        foreach (var i in safeIdx)
        {
            ...
            // CBT_WF_WHITELIST=0 lifts it, to measure what the shapes above would
            // contribute now that inlining no longer mints tautologies by capture.
            if (Environment.GetEnvironmentVariable("CBT_WF_WHITELIST") != "0"
                && (lits[i].Contains("old(") || lits[i].Contains("forall") || lits[i].Contains("exists")))
            { kept.Add(i); continue; }
            var wf = new List<string>();
            for (int j = 0; j < clause.Count; j++)
            {
