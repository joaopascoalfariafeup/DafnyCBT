...
static class SmtTranslator
{
    ...
    /// <summary>
    /// Compact, canonical shape signature for shape-matching across tests.
    /// Concatenates `<name>:<len>:<rank-vector>` over every int-typed
    /// seq/array input in `values`. Rank vector = the unique-under-
    /// monotonic-value-remap encoding (e.g. both [1,2,1,2] and [10,20,10,20]
    /// give ranks [0,1,0,1]; [1,1,2,2] gives [0,0,1,1] — different shape).
    /// Returns null when no applicable inputs.
    /// </summary>
        ...
        HashSet<string> mutableNames)
    {
        ...
        foreach (var (name, type) in inputs)
        {
            ...
            if (len == 1)
            {
                // Singleton: rank vector is always [0]; signature is just name:1:0.
                parts.Add($"{name}:1:0");
                continue;
            }
